...
def get_link_prediction_args(is_evaluation: bool = False):
    ...
    parser.add_argument('--model_name', type=str, default='DyGFormer', help='name of the model, note that EdgeBank is only applicable for evaluation',
                        choices=['JODIE', 'DyRep', 'TGAT', 'TGN', 'CAWN', 'EdgeBank', 'TCL', 'GraphMixer', 'DyGFormer','LTFDyG'])
    ...
    parser.add_argument('--auto_p', action='store_true', default=False,
                        help='Whether to automatically determine p based on data periodicity')
    parser.add_argument('--lete_p', type=float, default=0.5,
                        help='Fraction of dimensions for Fourier-based LeTE (0<=p<=1; 0.5=half Fourier, half Spline)')
    parser.add_argument('--lete_layer_norm', action='store_true', default=True,
                        help='Whether to use LayerNorm in LeTE (default: True)')
    parser.add_argument('--lete_scale', action='store_true', default=True,
                        help='Whether to use learnable scale in LeTE (default: True)')
    parser.add_argument('--lete_param_grad', action='store_false', default=True,
                        help='Whether LeTE parameters require gradient (default: True)')
    parser.add_argument('--spectral_entropy_calculation', choices=['global', 'per_node'], default='global', help='Choose the calculation method: global or per_node')
    try:
