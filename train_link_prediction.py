...
from models.LTFDyG import LTFDyG
...
from utils.periodicity_analysis import analyze_periodicity_global,analyze_periodicity_per_node
...
if __name__ == "__main__":
    ...
    if args.spectral_entropy_calculation == 'global':
        entropy, recommended_p = analyze_periodicity_global(train_data.node_interact_times)

    elif args.spectral_entropy_calculation == 'per_node':
        entropy, recommended_p = analyze_periodicity_per_node(
            train_data.src_node_ids,
            train_data.dst_node_ids,
            train_data.node_interact_times
        )

    args.lete_p = recommended_p
    ...
    for run in range(args.num_runs):
        ...
        elif args.model_name == 'LTFDyG':
            dynamic_backbone = LTFDyG(node_raw_features=node_raw_features, edge_raw_features=edge_raw_features, neighbor_sampler=train_neighbor_sampler,
                                         time_feat_dim=args.time_feat_dim, channel_embedding_dim=args.channel_embedding_dim, patch_size=args.patch_size,
                                         num_layers=args.num_layers, num_heads=args.num_heads, dropout=args.dropout,
                                         max_input_sequence_length=args.max_input_sequence_length, device=args.device,
                                         lete_p = recommended_p, lete_layer_norm = args.lete_layer_norm,
                                         lete_scale = args.lete_scale,
                                         lete_param_grad = args.lete_param_grad
                                         )
        else:
        ...
        for epoch in range(args.num_epochs):
            ...
            model.train()
            if args.model_name in ['DyRep', 'TGAT', 'TGN', 'CAWN', 'TCL', 'GraphMixer', 'DyGFormer','LTFDyG']:
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                if args.model_name in ['TGAT', 'CAWN', 'TCL']:
                    ...
                    batch_neg_src_node_embeddings, batch_neg_dst_node_embeddings = \
                        model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_neg_src_node_ids,
                                                                          dst_node_ids=batch_neg_dst_node_ids,
                                                                          node_interact_times=batch_node_interact_times)
                elif args.model_name in ['LTFDyG']:
                    # get temporal embedding of source and destination nodes
                    # two Tensors, with shape (batch_size, node_feat_dim)
                ...
                predicts = predicts.clamp(min=1e-7, max=1 - 1e-7)
            ...
            logging.info("\nLeTE Time Encoder Configuration:")
            logging.info(f"  - Time feature dimension: {args.time_feat_dim}")
            logging.info(f"  - Fourier dimension fraction (p): {args.lete_p}")
            logging.info(f"  - Use learnable scale: {args.lete_scale}")
